...
def save_formatted_plot(images, titles, filepath, cmap=None, colorbar_mode=None, vmin=None, vmax=None):
    ...
    if n == 1: 
        axes = [axes] # Standardize to list for iteration
    ...
    if colorbar_mode == 'shared':
        # List cast ensures compatibility across different Matplotlib versions
...
def run_part_1a():
    ...
    print("Part 1(a) plots saved.")

def run_part_1b():
    orig_random = plt.imread('./data/random.png')
    if orig_random.ndim == 3: orig_random = orig_random[:, :, 0] 
    ...
    # Apply "jet" colormap and display colorbar[cite: 1]
...
def run_part_1c_1d():
    ...
    print("Part 1(c) and 1(d) plots saved.")
...
def run_part_1e():
    ...
    img_rot_nn = np.clip(np.loadtxt('temp/1_e_rotated_nn.csv', delimiter=',').reshape((H_m, W_m, 3)), 0, 255).astype(np.uint8)
    img_rot_bi = np.clip(np.loadtxt('temp/1_e_rotated_bilinear.csv', delimiter=',').reshape((H_m, W_m, 3)), 0, 255).astype(np.uint8)
    ...
    save_formatted_plot([img_rot_nn], ["Rotated (NN)"], 'output_img/1_e_op_rotated_nn_standalone.png')
    save_formatted_plot([img_rot_bi], ["Rotated (Bilinear)"], 'output_img/1_e_op_rotated_bilinear_standalone.png')
    print("Part 1(e) plots saved.")
...
def run_part_1f():
    ...
    # Use identical limits for visual inference[cite: 1]
